...
# POS Secret Token (set in .env file)
$POS_SECRET = "your_secure_pos_secret_token_here"
...
curl "http://localhost:3000/api/pos/wishlists/$wishlistId/status" -H "x-pos-secret: $POS_SECRET" | ConvertFrom-Json | ConvertTo-Json

Write-Host "`n=== 6. Fetch Wishlist at POS (QR Scan) ===" -ForegroundColor Green
$fetchBody = @{qr_token = $qrToken} | ConvertTo-Json
$fetchResponse = curl -X POST "http://localhost:3000/api/pos/wishlists/$wishlistId/fetch" -H "Content-Type: application/json" -H "x-pos-secret: $POS_SECRET" -d $fetchBody | ConvertFrom-Json
...
curl -X POST "http://localhost:3000/api/pos/wishlists/$wishlistId/complete" -H "Content-Type: application/json" -H "x-pos-secret: $POS_SECRET" -d $completeBody | ConvertFrom-Json | ConvertTo-Json -Depth 10
